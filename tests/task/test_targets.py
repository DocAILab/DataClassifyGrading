...
def test_pers_info_without_corpus_still_generates_target() -> None:
    ...
    assert target.category_id.startswith("pers_info:")

    registry = build_leaf_registry(
        [
            resolver.resolve(
                _record({"level_1": "", "level_2": "", "level_3": "", "level_4": label})
            )
            for label in (
                "人力资源数据",
                "任课信息",
    ...
    # descriptions may be missing entirely (incomplete corpus)
    assert all(category.description == "" for category in registry.categories)


# --- 4. category_id deterministic ---------------------------------------------


def test_stable_category_id_is_deterministic_and_whitespace_normalized() -> None:
    a = stable_category_id("finance", ["业务", "账户信息", "", "配置信息"])
    b = stable_category_id("finance", ["业务", "账户信息", "", "配置信息"])
    assert a == b
    spaced = stable_category_id("finance", ["经营 管理", "技术管理", "系统管理信息", "配置信息"])
    compacted = stable_category_id("finance", ["经营管理", "技术管理", "系统管理信息", "配置信息"])
    assert spaced == compacted
    assert a != spaced
    assert a.count(":") == 1 and len(a.rsplit(":", 1)[1]) == 16
