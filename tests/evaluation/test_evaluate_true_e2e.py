"""TRUE end-to-end evaluator unit tests (Phase 13.5).

Drives ``evaluate_true_e2e.run_one`` / ``aggregate_true_e2e`` on CPU with an
injected ``generate`` function (canned model output keyed on whether the
prompt is Stage 1 catalog or Stage 2 candidate bundle). Covers:
...
Identity/decode/prompt logic is the frozen shared layer (PromptChoiceRegistry,
build_stage1/2_prompt, check_stage1/2_choices); nothing is re-implemented here.
"""
